...
- [ ] Formalize data access
  - All data must be obtained via the state to guarantee program soundness
    - ex. Due to global state and differing entrypoints, various RoboDolg elements are currently using information that goes out of sync with GoldenOrb
...
- [ ] Reverse-engineer vanilla camera positioning
  - Seems to have some offset relative to the hands' rotation origin
  - Will need to position camera relative to ~bip01~
- [ ] Proper support for camera roll
